/**
 * Tool Parser Module
 * Ported from Chat2API utils/toolParser.ts
 *
...
function parseToolCallsFromText(text, modelType = 'default') {
  ...
  while ((blockMatch = blockRegex.exec(processedText)) !== null) {
    ...
    if (modelType === 'kimi' || modelType === 'glm' || modelType === 'minimax' || modelType === 'zai') {
      ...
      // Modern Balanced-Braces Logic (for Qwen and others)
    ...
    // If the block is now empty, remove the tags too
...
export { parseToolCallsFromText, extractBalancedJson, tryParseJSON, tryRegexFallback }
